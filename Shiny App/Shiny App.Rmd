...
shinyApp(
  ui = fluidPage(navbarPage("Data Science Related Courses at UU", theme = shinytheme("paper"),
                  ...
                  tabPanel("Facet overview",
                           column(width = 12,
                                  wellPanel(
                                      h6("THere on the left you van see the university overview of the")
                                      ),
                           wellPanel(
...
output$plots2 <- renderPlotly({
  ...
  ggplotly(
    plot1_perc %>% ggplot(aes(x=total,y=name,fill=Faculty))+geom_col()+ylab("")+xlab("percentage (%)")+theme_minimal()
